...
class MountainCar():
    ...
    def update_table(self, old_state, action, reward, new_state, alpha):
        ...
        self.Q_table[tuple(old_state)][action] += alpha * (
                    reward + self.gamma * new_state_Q_value - self.Q_table[tuple(old_state)][action])
    ...
    def run(self):
        ...
        plt.title('Episode Histogram of Mountain Car problem solving by Q Learning')
        plt.show()
        plt.hist(scores, 50, normed=1, facecolor='g', alpha=0.75)
        plt.xlabel('Rewards Achieved Per Episode')
        plt.ylabel('Frequency')
        plt.title('Rewards Histogram of Mountain Car problem solving by Q Learning')
        plt.show()
        plt.hist(penalties_result, 50, normed=1, facecolor='g', alpha=0.75)
        plt.xlabel('Penalties Per Episode')
        plt.ylabel('Frequency')
        plt.title('Penalties Histogram of Mountain Car problem solving by Q Learning')
        plt.show()
